...
def send_main_menu(message):
    ...
    current_button = types.KeyboardButton('Текущие задачи')
    completed_button = types.KeyboardButton('Выполненные задачи')
    keyboard.add(current_button, completed_button)
    bot.send_message(message.chat.id, "Выберите действие:", reply_markup=keyboard)


@bot.message_handler(func=lambda message: message.text == 'Текущие задачи')
def show_current_reminders(message):
    ...
    if reminders:
        ...
        bot.send_message(message.chat.id, "Нет текущих задач")
...
def handle_edit_period_query(query):
    ...
    msg = bot.send_message(query.message.chat.id, "Укажите новую периодичность напоминания в формате dd hh mm:")
...
def edit_files_handler(call):
    ...
    try:
        ...
        bot.send_message(chat_id, "Нет вложенных файлов")
        return
...
def delete_file_handler(call):
    ...
    if delete_file_from_database(user_id, file_id, reminder_id):
        bot.send_message(call.message.chat.id, f"Файл (ID={file_id}) успешно удален")
    else:
        bot.send_message(call.message.chat.id, f"Ошибка при удалении файла (ID={file_id})")
...
def process_edit_date(message, user_id, reminder_id):
    global value_new
    msg = bot.send_message(message.chat.id, "Введите новое время (в формате hh:mm):")
...
def process_edit_date1(message, user_id, reminder_id, value_new):
    if not validate_time_format(message.text):
        msg = bot.send_message(message.chat.id, "Некорректные данные. Введите новое время (в формате hh:mm):")
...
def process_edit_time(message, user_id, reminder_id, new_date):
    ...
    msg = bot.send_message(message.chat.id, "Дата и время обновлены")
    process_return(msg)
...
@bot.message_handler(func=lambda message: message.text == 'Выполненные задачи')
...
def process_return(message):
    bot.send_message(message.chat.id, "Напоминание успешно обновлено и возвращено")
...
def start(message):
    ...
    welcome_message = (
        ...
        "Я бот Маркова Михаила, студента группы 5030102/10201. Я помогу "
...
def cal(c):
    ...
    if not result and key:
        ...
        if values is not None:
            msg = bot.send_message(c.message.chat.id, f"Выберите время в формате hh:mm")
...
def set_time(message, chosen_date, text):
    try:
        ...
        if not validate_time_format(time_chosen):
            msg = bot.send_message(chat_id, "Некорректные данные. Введите новое время (в формате hh:mm):")
...
def handle_periodic_yes(call):
    chat_id = call.message.chat.id
    bot.send_message(chat_id, "Укажите периодичность в формате dd hh mm")
...
def ask_periodic_interval(message, id=None, only_edit=False):
    ...
    try:
        ...
        bot.send_message(chat_id, f"Напоминание установлено с интервалом {period}.")
        if id is None:
        ...
        msg = bot.send_message(chat_id, "Некорректные данные. Введите новое время (в формате dd hh mm):")
...
def handle_periodic_no(call):
    chat_id = call.message.chat.id
    bot.send_message(chat_id, "Напоминание будет озвучено единожды")
...
def handle_attachment(call):
    ...
    if call.data.startswith('attach_yes'):
        ...
        flag = True
        msg = bot.send_message(chat_id, "Отправьте вложения. Затем введите end")
...
def delete_file_from_drive(file_id):
    ...
    try:
        ...
        print("Файл успешно удален с Google Drive")
    except Exception as e:
        print("Ошибка при удалении файла с Google Drive:", e)
